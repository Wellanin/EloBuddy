...
namespace SonaEB
{
    ...
    //    }
        ...
        public static Menu Menu, SettingsMenu, DrawMenu, MiscMenu;
        ...
        private static void Loading_OnLoadingComplete(EventArgs args)
        {
           ...
           // SettingsMenu.Add("E_Combo", new CheckBox("Use E on Combo"));
            ...
            SettingsMenu.Add("Q_LaneClear", new CheckBox("Use Q on LaneClear"));
            
            SettingsMenu.AddLabel("LastHit");
            SettingsMenu.Add("Q_LastHit", new CheckBox("Use Q on LastHit"));
            
            MiscMenu = Menu.AddSubMenu("Misc");
          
            
            MiscMenu.Add("AntiGapCloser", new CheckBox("Anti GapCloser"));
            MiscMenu.Add("SmartHeal", new CheckBox("Smart Auto W"));
        ...
        public static void OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
        		...
        		if (sender.IsEnemy  && !Player.IsDead
        		    && (args.Target.IsMe || (args.Target.IsAlly && Player.Distance(args.Target) < W.Range-50) ) )
        		{
        			...
        			var attackedHero = ObjectManager.Get<Obj_AI_Base>().First(hero => hero.NetworkId == args.Target.NetworkId);
        			
        			if (sender.Type == GameObjectType.AIHeroClient && MiscMenu["SaveAllyR"].Cast<CheckBox>().CurrentValue && R.IsReady() && attackerHero.Distance(Player) < R.Range-50)
        			{
        			...
        			{
        				if (attackedHero.HealthPercent < 70){
        					W.Cast();
        					Chat.Print(55 );
        				}
        				
        				
        				if (attackedHero.HealthPercent < 90 && Player.ManaPercent > 85){
        					W.Cast();
        					Chat.Print(66 );
        				}
        ...
        private static void Game_OnTick(EventArgs args)
        {
        	...
        	if (MiscMenu["SmartHeal"].Cast<CheckBox>().CurrentValue && W.IsReady())
        	{
        		var ally = EntityManager.Heroes.Allies
        			.Where(x => x.HealthPercent < 25 && x.Distance(Player) < W.Range)
        			.OrderBy( x => x.Health);
        		if (ally != null)
        		{
        			Chat.Print(ally.FirstOrDefault().ChampionName);
        			W.Cast();
        		}
        ...
        private static void Combo()
        {
            ...
            var useE = SettingsMenu["E_Combo"].Cast<CheckBox>().CurrentValue;
          ...
          /*
            ...
            if (useE && E.IsReady()&& !target.HasBuff("AkaliMota") && !Player.IsDashing() && !target.IsDead && !target.IsZombie && target.IsValidTarget(325))
            {
                E.Cast();
            }
            */
         ...
         private static void LastHit()
        {
         	...
         	if (SettingsMenu["Q_LastHit"].Cast<CheckBox>().CurrentValue && Q.IsReady())
        	{
        		var	minions = ObjectManager.Get<Obj_AI_Minion>()
        			.Where(x => x.Distance(Player) < Q.Range && Player.GetSpellDamage(x,SpellSlot.Q,DamageLibrary.SpellStages.Default) > x.Health)
